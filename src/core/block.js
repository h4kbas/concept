...
const block_explorer_1 = require("./block-explorer");
/**
...
class Block {
    constructor(config = {}) {
        ...
        this._blockReferences = new Map();
        this._hookMap = config.hookMap || {};
        this._blockExplorer = new block_explorer_1.BlockExplorer(this);
     ...
     */
    isPairAvailable(conceptA, conceptB) {
        const key = this._getPairKey(conceptA, conceptB);
        return this._pairs.has(key);
    }
    addPair(conceptAOrPair, conceptB) {
        let pair;
        if (conceptB) {
            // Called with two concepts
            pair = { conceptA: conceptAOrPair, conceptB };
        }
        else {
    ...
    addData(pairOrData, value) {
        let data;
        if (value !== undefined) {
            // Called with pair and value
            data = { pair: pairOrData, value };
        }
        else {
        ...
        const existingData = this._chain.find(d => d.pair.conceptA.name === data.pair.conceptA.name &&
     ...
     */
    addToChain(conceptA, conceptB, relation, relationshipType) {
        const pair = this.addPair(conceptA, conceptB);
        const data = relationshipType
    ...
    inferMissingPairs() {
        ...
        for (const pair of pairsArray) {
            ...
            const dependentPairs = pairsArray.filter(p => p.conceptB.name === conceptA.name);
            for (const dependentPair of dependentPairs) {
                ...
                const inferredPairKey = this._getPairKey(dependentPair.conceptA, conceptB);
                if (this._pairs.has(inferredPairKey)) {
                    continue;
                }
                // Calculate the states of the relevant pairs
                const dependentPairState = this._blockExplorer.calculateCurrentPairState(dependentPair);
                ...
                if (dependentPairState !== null) {
                    ...
                    const dependentPairData = this.chain.find(data => data.pair.conceptA.name === dependentPair.conceptA.name &&
                        data.pair.conceptB.name === dependentPair.conceptB.name);
                    if (dependentPairData?.relationshipType === 'has') {
                        // Skip inference for 'has' relationships
                        continue;
                    }
                    const inferredValue = currentPairState === true
                        ? dependentPairState
                        : !dependentPairState;
     ...
     */
    calculateHookResult(token, line, block) {
     ...
     */
    groupConceptsIntoLines(concepts) {
        const lines = [];
        let currentLine = [];
     ...
     */
    groupConceptsIntoRelationshipLines(concepts) {
        const lines = [];
     ...
     */
    isReference(conceptName) {
        return (this._references.has(conceptName) ||
            this._blockReferences.has(conceptName));
    }
    /**
     * Get all references for a concept
     */
    getReferences(conceptName) {
        const references = [];
     ...
     */
    resolveWithResolver(conceptName, resolver) {
